...
class Ban extends Command {
   ...
   */
	...
	async run(message, args, prefix) {
		...
		if (user.id === this.client.user.id) return message.reply({ content: 'I refuse to be banned' });
		...
		if (userRolePossition <= rolePosition) return message.reply({ content: 'Cannot ban that member because they have roles that is higher or equal to you.' });
		if (botRolePossition <= rolePosition) return message.reply({ content: 'Cannot ban that member because they have roles that is higher or equal to me.' });
		if (!user.bannable) {
